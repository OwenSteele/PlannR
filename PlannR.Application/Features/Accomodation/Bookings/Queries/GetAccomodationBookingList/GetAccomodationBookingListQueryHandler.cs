...
{
    public class GetAccomodationBookingListQueryHandler : IRequestHandler<GetAccomodationBookingListQuery, ICollection<AccomodationBookingListDataModel>>
    {
        ...
        public async Task<ICollection<AccomodationBookingListDataModel>> Handle(GetAccomodationBookingListQuery request, CancellationToken cancellationToken)
        {
            var result = (await _accomodationBookingRepository.ListAllAsync()).OrderBy(x => x.Name).ToList();

            var authorisedResult = _authorisationService.RemoveInAccessibleEntities(result);

            return _mapper.Map<ICollection<AccomodationBookingListDataModel>>(authorisedResult);
        }
    }
}
